...
//بر اساس Edge Tunnel 
//برای همه اپراتور ها تنظیم شده است ؛ مخابرات ، همراه اول ، ایرانسل ، رایتل و ...

let userID = '39ea49d7-edcd-4b67-b5a0-2a376de04e67';

// بهترین آدرس آیپی تمیز
const bestCFIP = "www.fivesecure.net"

// جهت پشتیبانی از آیپی های خود کلودفلر
...
function getVLESSConfig(userID, hostName) {
    const vlessLink = `vless://${userID}\u0040${bestCFIP}:80?encryption=none&security=none&fp=randomized&type=ws&host=${hostName}&path=%2F%3Fed%3D2560#KayHGNG-Workers`
    const vlessTlsLink = `vless://${userID}\u0040${bestCFIP}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2F%3Fed%3D2560#KayHGNG-Workers`
    return `
در زیر اطلاعات گره و پیوند به اشتراک گذاری گره از پورت غیر TLS است. می توانید از پورت غیر TLS پشتیبانی شده توسط CF استفاده کنید.

آدرس: ${hostName} یا IP ترجیحی CF
پورت: پورت 80 یا غیر TLS که توسط CF پشتیبانی می شود
UUID：${userID}
نوع：ws
آدرس ورکر：${hostName}
path：/?ed=2048
...
در زیر اطلاعات گره و پیوند اشتراک گذاری گره درگاه TLS آمده است. می توانید از پورت TLS پشتیبانی شده توسط CF استفاده کنید.

آدرس: ${hostName} یا IP ترجیحی CF
پورت: 443 یا پورت TLS با پشتیبانی از CF
UUID：${userID}
نوع：ws
امنیت：TLS
آدرس ورکر：${hostName}
...
Base64 لینک ：https://${hostName}/${userID}/base64
Clash لینک：https://${hostName}/${userID}/clash
Sing-box لینک：https://${hostName}/${userID}/sb

نکته: در برخی مناطق، نام دامنه پیش‌فرض CF آلوده است، مگر اینکه عملکرد تکه تکه‌سازی TLS  روشن باشد، گره پورت TLS قابل استفاده نیست.
برای مثال، گره‌های مستقر برای Pages فقط می‌توانند از گره‌هایی با پورت‌های TLS استفاده کنند.
---------------------------------------------------------------
`;
}
